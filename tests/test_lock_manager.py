...
def test_lock_manager_uses_context_manager_api_only() -> None:
    assert hasattr(LockManager, 'event_lock')
    assert hasattr(LockManager, 'handler_lock')
    assert hasattr(LockManager, 'handler_dispatch_context')
...
async def test_handler_dispatch_context_preserves_reentrant_locking_in_dispatch_context() -> None:
    ...
    async with lock_manager.event_lock(bus, event):
        ...
        async def lock_with_dispatch_mark() -> str:
            with lock_manager.handler_dispatch_context(bus, event):
                async with lock_manager.event_lock(bus, event):
                    return 'acquired'
