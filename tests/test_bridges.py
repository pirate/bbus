...
def _normalize_roundtrip_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized.pop('event_result_schema', None)
    return normalized
...
async def _wait_for_path(path: Path, *, process: subprocess.Popen[str], timeout: float = 15.0) -> None:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if path.exists():
            return
...
def _make_sender_bridge(kind: str, config: dict[str, Any]) -> Any:
    if kind == 'http':
...
async def _assert_roundtrip(kind: str, config: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory(prefix=f'bubus-bridge-{kind}-') as temp_dir:
        ...
        try:
            ...
            if kind == 'postgres':
                await sender.start()
            outbound = IPCPingEvent(value=17, label=f'{kind}_ok', meta={'kind': kind, 'n': 1})
