...
test('500 ephemeral buses with 100 events each', { timeout: 30_000 }, async () => {
  ...
  for (let b = 0; b < total_buses; b += 1) {
    const bus = new EventBus(`ReqBus-${b}`, { max_history_size: 10 })
...
test('50k events with ephemeral on/off handler registration across 2 buses', { timeout: 30_000 }, async () => {
  ...
  const bus_a = new EventBus('SharedBusA', { max_history_size: 1000 })
  const bus_b = new EventBus('SharedBusB', { max_history_size: 1000 })
...
// _getBusScopedEvent, ephemeral on/off handler churn, find() waiter timeouts,
// and aggressive history trimming via _gc(). If any code path leaks references,
// memory will grow unbounded across 2000 iterations.
test(
  'worst-case: forwarding + queue-jump + timeouts + cancellation at scale',
  { timeout: 60_000 },
  async () => {
    ...
    const bus_a = new EventBus('WC_A', { max_history_size: 50 })
    const bus_b = new EventBus('WC_B', { max_history_size: 50 })
    const bus_c = new EventBus('WC_C', { max_history_size: 50 })
    ...
    assert.ok(bus_a.event_history.size <= 50, `bus_a history ${bus_a.event_history.size} > 50`)
    assert.ok(bus_b.event_history.size <= 50, `bus_b history ${bus_b.event_history.size} > 50`)
    assert.ok(bus_c.event_history.size <= 50, `bus_c history ${bus_c.event_history.size} > 50`)
    ...
    assert.ok(
      mem_delta_mb < 50,
      `Memory grew ${mem_delta_mb.toFixed(1)}MB over baseline (limit 50MB)`
    )
