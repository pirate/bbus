...
test("handler error is captured and does not prevent other handlers from running", async () => {
  ...
  const failing_result = Array.from(event.event_results.values()).find(
    (r) => r.handler_name === "failing_handler"
  );
  ...
  assert.ok(
    (failing_result.error as Error).message.includes("Expected to fail"),
    "error message should contain the thrown message"
  );

  const working_result = Array.from(event.event_results.values()).find(
    (r) => r.handler_name === "working_handler"
  );
