...
test("circular forwarding does not cause infinite loop", async () => {
  ...
  bus_a.on(CircEvent, () => { handler_calls.push("A"); return "a"; });
  bus_b.on(CircEvent, () => { handler_calls.push("B"); return "b"; });
  bus_c.on(CircEvent, () => { handler_calls.push("C"); return "c"; });
...
test("off() with no handler removes all handlers for that event", async () => {
  ...
  bus.on(OffAllEvent, () => { call_count_a += 1; });
  bus.on(OffAllEvent, () => { call_count_b += 1; });
  bus.on(OtherEvent, () => { other_count += 1; });
