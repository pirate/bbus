...
test("event.bus returns correct bus when multiple buses exist", async () => {
  ...
  bus1.dispatch(MainEvent({}));
  await bus1.waitUntilIdle();

  bus2.dispatch(MainEvent({}));
...
test("event.bus reflects the currently-processing bus when forwarded", async () => {
  ...
  const event = bus1.dispatch(MainEvent({}));
  await bus1.waitUntilIdle();
  await bus2.waitUntilIdle();
...
test("event.bus.dispatch sets parent-child relationships through 3 levels", async () => {
  const bus = new EventBus("MainBus");
  ...
  const parent_event = bus.dispatch(MainEvent({}));
...
test("event.bus with forwarding: child dispatched via event.bus goes to the correct bus", async () => {
  ...
  const parent_event = bus1.dispatch(MainEvent({}));
  await bus1.waitUntilIdle();
  await bus2.waitUntilIdle();
...
test("event.bus is set on the event after dispatch (outside handler)", async () => {
  ...
  const dispatched = bus.dispatch(raw_event);
  assert.ok(dispatched.bus, "event.bus should be set after dispatch");

  await bus.waitUntilIdle();
});

test("event.bus.dispatch from handler correctly attributes event_emitted_by_handler_id", async () => {
  const bus = new EventBus("TestBus");

  let child_emitted_by_handler_id: string | undefined;
  ...
  assert.ok(
    child!.event_emitted_by_handler_id,
    "event_emitted_by_handler_id should be set on child events dispatched via event.bus"
  );
