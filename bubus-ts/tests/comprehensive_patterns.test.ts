...
test('isInsideHandler() is per-bus, not global', async () => {
  ...
  assert.equal(bus_a_inside_during_a_handler, true, 'bus_a.isInsideHandler() should be true during bus_a handler')
  assert.equal(bus_b_inside_during_a_handler, false, 'bus_b.isInsideHandler() should be false during bus_a handler')

  // During bus_b's handler: bus_b should report inside, bus_a should not
  assert.equal(bus_b_inside_during_b_handler, true, 'bus_b.isInsideHandler() should be true during bus_b handler')
  assert.equal(bus_a_inside_during_b_handler, false, 'bus_a.isInsideHandler() should be false during bus_b handler')
