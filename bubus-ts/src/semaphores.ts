...
export const resolveConcurrencyMode = (
  mode: ConcurrencyMode | undefined,
  fallback: ConcurrencyMode
): ConcurrencyMode => {
...
export const limiterForMode = (
  mode: ConcurrencyMode,
  global_limiter: AsyncLimiter,
  bus_limiter: AsyncLimiter
): AsyncLimiter | null => {
  if (mode === "parallel") {
    return null;
  }
...
export const runWithLimiter = async <T>(
  limiter: AsyncLimiter | null,
  fn: () => Promise<T>
): Promise<T> => {
  if (!limiter) {
    return await fn();
