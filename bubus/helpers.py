...
def create_model_from_schema(schema: dict[str, Any]) -> type[BaseModel]:  # noqa: C901
    ...
    def resolve_field_type(field_schema: dict[str, Any]) -> Any:
        """Resolve field type, including optional types and nullability."""
        if '$ref' in field_schema:
            model_reference = str(field_schema['$ref']).split('/')[-1]
            return models.get(model_reference, Any)

        any_of_raw = field_schema.get('anyOf')
        if isinstance(any_of_raw, Sequence) and not isinstance(any_of_raw, (str, bytes, bytearray)):
            any_of_candidates = cast(Sequence[Any], any_of_raw)
            any_of_types: list[Any] = []
            for candidate_raw in any_of_candidates:
                candidate = _as_string_key_dict(candidate_raw)
                if candidate is None:
                    continue
                candidate_type = candidate.get('type')
                if isinstance(candidate_type, str) and candidate_type in TYPE_MAPPING:
                    resolved = TYPE_MAPPING[candidate_type]
                    if resolved is not type(None):
                        any_of_types.append(resolved)
        ...
        field_type = (
            TYPE_MAPPING[field_type_name] if isinstance(field_type_name, str) and field_type_name in TYPE_MAPPING else Any
        )
    ...
    definitions = _as_string_key_dict(schema.get('$defs'))
    if definitions is not None:
        for model_name, model_schema_raw in definitions.items():
            model_schema = _as_string_key_dict(model_schema_raw)
            if model_schema is None:
                continue
            fields: dict[str, tuple[Any, Any]] = {}
            properties = _as_string_key_dict(model_schema.get('properties'))
            if properties is not None:
                for field_name, field_schema_raw in properties.items():
                    ...
                    fields[str(field_name)] = (field_type, Field(**field_params))

            field_definitions: dict[str, Any] = {field_name: field_definition for field_name, field_definition in fields.items()}
            models[str(model_name)] = create_model(
                str(model_name),
    ...
    main_fields: dict[str, tuple[Any, Any]] = {}
    ...
    if properties is not None:
        for field_name, field_schema_raw in properties.items():
            ...
            field_params = get_field_params_from_field_schema(field_schema=field_schema)
            main_fields[str(field_name)] = (field_type, Field(**field_params))

    main_field_definitions: dict[str, Any] = {
        field_name: field_definition for field_name, field_definition in main_fields.items()
    }
    return create_model(
        str(schema.get('title', 'MainModel')),
        __doc__=str(schema.get('description', '')),
