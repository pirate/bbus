...
async with eventbus.locks.with_handler_lock(eventbus, event, event_result):
    with eventbus.with_handler_execution_context(event, event_result.handler_id):
...
await this.locks.withEventLock(event, () =>
  this._runHandlersWithTimeout(event, pending_entries, resolved_event_timeout, () =>
    runWithSlowMonitor(event._createSlowEventWarningTimer(), () =>
...
await this.bus.locks.withHandlerExecutionContext(this, async () => {
  await runWithAsyncContext(event._getDispatchContext() ?? null, async () => {
    await runWithTimeout(this.handler_timeout, () => this._createHandlerTimeoutError(event), () =>
