...
- `eventbus.with_handler_execution_context(...)`
- `with_timeout(...)`
- `with_slow_monitor(...)`
- `call_handler(...)`
...
- `withHandlerLock(...)` (acquired before `runHandler(...)`)
- `withHandlerExecutionContext(...)`
- `runWithTimeout(...)`
- `runWithSlowMonitor(...)`
- handler call (internally wrapped by `runWithAsyncContext(...)` + `runWithAbortMonitor(...)` for context propagation and cancellation)
