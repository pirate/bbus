...
`EventBus` is the central runtime for handler registration, event dispatch, history lookup, and lifecycle control.
...
| `max_history_drop` | If `true`, drop oldest history entries when full; if `false`, reject new dispatches at limit. |
...
## Dispatch and emission

`dispatch(...)` enqueues synchronously and returns the pending event immediately. `emit(...)` is an alias.

<Tabs>
<Tab title="Python">

```python
event = bus.dispatch(MyEvent(data='x'))
...
```ts
const event = bus.dispatch(MyEvent({ data: 'x' }))
